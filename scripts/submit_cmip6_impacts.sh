...
RUN_ROOT="${LPS_CMIP6_IMPACT_ROOT:-$ATLAS_ROOT/.cmip6-runs/climate-impact-production}"
...
if [[ -s "$RUN_ROOT/run.job-id" && "${LPS_CMIP6_IMPACT_FORCE_SUBMIT:-0}" != "1" ]]; then
  printf 'A CMIP6 impact submission is already recorded in %s; set LPS_CMIP6_IMPACT_FORCE_SUBMIT=1 for a resumable retry.\n' "$RUN_ROOT"
  exit 0
fi

cd "$ATLAS_ROOT"
"$PYTHON" -m cmip6_pipeline.impact plan --run-root "$RUN_ROOT" --geometry-asset "$GEOMETRY" \
  --pair-root "$ATLAS_ROOT/.cmip6-runs/mri-paired" \
  --pair-root "$ATLAS_ROOT/.cmip6-runs/mpi-esm1-2-hr-production" \
  --pair-root "$ATLAS_ROOT/.cmip6-runs/hadgem-ll-paired" \
  --pair-root "$ATLAS_ROOT/.cmip6-runs/mpi-lr-paired" \
  --pair-root "$ATLAS_ROOT/.cmip6-runs/miroc6-paired"
...
printf 'CMIP6 precipitation impacts: runs %s, pairs %s\n' "$RUN_ID" "$PAIR_ID"
