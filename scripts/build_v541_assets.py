#!/usr/bin/env python3
"""Build compact browser assets from the LPS v5.4.1 recall-first core catalogue."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--qa-amendment", required=True, type=Path)
...
def main() -> None:
    ...
    qa_amendment = json.loads(args.qa_amendment.read_text(encoding="utf-8"))
    ...
    if ibtracs and ibtracs.get("schema") != "lps-v5.4.1-ibtracs-v04r01-parent-crosswalk-v1":
        raise ValueError("Unsupported IBTrACS crosswalk schema")
    if release.get("schema") != "lps-v5.4.1-release-candidate-manifest-v1":
        raise ValueError("Unsupported release manifest")
    if metadata.get("version") != "5.4.1" or qa_release.get("status") != "pass":
        raise ValueError("v5.4.1 metadata or final QA does not report a passing release")
    if qa_amendment.get("scientific_change_to_catalogue") is not False:
        raise ValueError("Unexpected scientific catalogue change in the QA amendment")
    ...
    if data["track_id"].nunique() != int(expected["publication_segments"]):
        raise ValueError("Publication-segment count does not match the release manifest")
    if data["continuity_parent_track_id"].nunique() != int(expected["parent_events"]):
        raise ValueError("Parent-event count does not match the release manifest")
    ...
    core = {
        "meta": {
            "title": "LPS v5.4.1 ERA5 recall-first core South Asian low-pressure-system catalogue",
            "rows": int(len(data)),
            ...
            "publication_splits": int(release["continuity"]["publication_splits"]),
            ...
            "row_grain": "One row per hourly LPS publication-segment position/time: observed detector fix or interpolated centre.",
            "identity_grain": "continuity_parent_track_id is one linked meteorological event; track_id is a gap-safe plotting segment.",
            "source_dataset": "ERA5-derived LPS v5.4.1 recall-first core catalogue",
            "catalogue_version": "v5.4.1",
            ...
            "qa_amendment_sha256": sha256(args.qa_amendment),
            "sources": {
                "live_atlas": "https://kieranmrhunt.github.io/monsoon-low-atlas/",
                "release_summary": "data/lps-v5.4.1-release-manifest.json",
                "metadata": "data/lps-v5.4.1-metadata.json",
                "qa": "data/lps-v5.4.1-qa.json",
                "qa_amendment": "data/lps-v5.4.1-qa-amendment.json",
                "quality_report": "data/lps-v5.4.1-quality-report.md",
    ...
    detail = {
        ...
        "state_rainfall": {
            ...
            "units": "mm day-1",
            "statistic": "Area-mean daily rainfall averaged over UTC calendar days touched by each LPS parent event.",
            "coverage": [1940, 2025],
    ...
    if manifest["qa"]["publication_splits"] != int(release["continuity"]["publication_splits"]):
        raise ValueError("Encoded publication-split count does not match the release manifest")
