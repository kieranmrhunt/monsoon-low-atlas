#!/usr/bin/env python3
"""Build compact browser assets from the LPS v5.4 ERA5 core catalogue."""
...
PARQUET_COLUMNS = [
    ...
    "is_mature_detection",
    "imd_category_raw_v54rean",
    "max_vort_smoothed_v54rean",
    "precip_24hr_v54rean",
    "max_wind_v54rean",
    "min_mslp_v54rean",
    "pressure_deficit_hpa_v54rean",
    "q850_mean_gkg_v54rean",
    "rh850_mean_pct_v54rean",
    "t850_mean_k_v54rean",
    "physics_complete_v54rean",
    "track_n_observations",
    "track_elapsed_span_hours",
    "track_max_gap_hours",
    "track_max_missing_run_hours",
    "track_stitch_count",
    "track_occupancy_fraction",
    "track_speed_ms",
]

FIELD_ALIASES = {
    "lon_smooth_global_v54": "lon",
    "lat_smooth_global_v54": "lat",
    "imd_category_raw_v54rean": "imd_category",
    "max_vort_smoothed_v54rean": "max_vort_smoothed",
    "precip_24hr_v54rean": "precip_24hr",
    "max_wind_v54rean": "max_wind",
    "min_mslp_v54rean": "min_mslp",
    "pressure_deficit_hpa_v54rean": "pressure_deficit_hpa",
    "q850_mean_gkg_v54rean": "q850_mean_gkg",
    "rh850_mean_pct_v54rean": "rh850_mean_pct",
    "t850_mean_k_v54rean": "t850_mean_k",
}
...
def state_rainfall_series(payload: dict, state_slugs: list[str]) -> tuple[list[dict[int, np.ndarray]], list[bool]]:
...
def mean_state_rainfall(group: pd.DataFrame, state_series: list[dict[int, np.ndarray]]) -> tuple[list[int], int]:
...
def main() -> None:
    ...
    rainfall_series, state_available = state_rainfall_series(rainfall, template["state_slugs"])
    ibtracs = (
        json.loads(args.ibtracs_crosswalk.read_text(encoding="utf-8"))
        if args.ibtracs_crosswalk
        else None
    )
    if ibtracs and ibtracs.get("schema") != "lps-v5.4-ibtracs-v04r01-crosswalk-v1":
        raise ValueError("Unsupported IBTrACS crosswalk schema")

    table = pq.read_table(args.parquet, columns=PARQUET_COLUMNS)
    data = table.to_pandas()
    data.rename(columns=FIELD_ALIASES, inplace=True)
    data["time"] = pd.to_datetime(data["time"], utc=True)
    data.sort_values(["track_id", "time"], kind="mergesort", inplace=True)
    data["month"] = data["time"].dt.month

    expected_rows = metadata["rows"]
    if sha256(args.parquet) != metadata["sha256"]:
        raise ValueError("Parquet SHA-256 does not match the v5.4 metadata")
    if len(data) != int(expected_rows["total"]):
        raise ValueError(f"Parquet row count {len(data)} != metadata {expected_rows['total']}")
    if data["track_id"].nunique() != int(metadata["tracks"]):
        raise ValueError("Parquet track count does not match the v5.4 metadata")
    ...
    if int(diagnostics.sum()) != int(expected_rows["observed"]):
        raise ValueError("Observed-row count does not match the v5.4 metadata")
    if int((~diagnostics).sum()) != int(expected_rows["posterior"]):
        raise ValueError("Interpolated-row count does not match the v5.4 metadata")
    if set(data.loc[diagnostics, "position_source"]) != {"observed"}:
        raise ValueError("Observed support and position_source disagree")
    if set(data.loc[~diagnostics, "position_source"]) != {"interpolated"}:
        raise ValueError("Interpolated support and position_source disagree")
    physics = [
        "imd_category",
        "max_vort_smoothed",
        "precip_24hr",
    ...
    if not data["physics_complete_v54rean"].astype(bool).all():
        raise ValueError("v5.4 contains positions without complete published-centre physics")
    if data[physics].isna().any().any():
        raise ValueError("v5.4 positions are missing required published-centre physics")

    groups = {int(track_id): group for track_id, group in data.groupby("track_id", sort=True)}
    summary_records: list[dict] = []
    for track_id, group in groups.items():
        observed_positions = int(group["candidate_diagnostics_available"].astype(bool).sum())
        reported_observations = int(group.iloc[0]["track_n_observations"])
        if observed_positions != reported_observations:
            raise ValueError(f"Track {track_id} observed-position count is inconsistent")
        summary_records.append(
            {
                "track_id": track_id,
                "genesis_year": int(group.iloc[0]["time"].year),
                "n_rows": len(group),
                "duration_hours_observed": finite(group.iloc[0]["track_elapsed_span_hours"]),
                "max_imd_category": int(round(float(group["imd_category"].max()))),
                "occupancy_fraction": finite(group.iloc[0]["track_occupancy_fraction"]),
                "posterior_row_fraction": 1 - observed_positions / len(group),
                "max_gap_hours": finite(group.iloc[0]["track_max_gap_hours"]),
                "max_missing_run_hours": finite(group.iloc[0]["track_max_missing_run_hours"]),
                "genesis_lat": finite(group.iloc[0]["lat"]),
                "genesis_lon": finite(group.iloc[0]["lon"]),
                "lysis_lat": finite(group.iloc[-1]["lat"]),
                "lysis_lon": finite(group.iloc[-1]["lon"]),
                "observed_positions": observed_positions,
                "qualifying_positions": int(group["is_mature_detection"].fillna(False).astype(bool).sum()),
                "stitch_count": int(group.iloc[0]["track_stitch_count"]),
            }
        )
    summary = pd.DataFrame(summary_records).sort_values(["track_id"], kind="mergesort")
    ...
    for index, cached in enumerate(group_cache):
        ...
        available_means = [(state_index, value) for state_index, value in enumerate(state_means) if value >= 0]
        top_state_idx, top_state_mean = max(available_means, key=lambda item: item[1]) if available_means else (-1, -1)
    ...
    for state_index in range(state_matrix.shape[1]):
        ...
        if not valid.any():
            continue
        ranks = pd.Series(state_matrix[valid, state_index]).rank(method="average", pct=True) * 100
        state_percentiles[valid, state_index] = np.rint(ranks.to_numpy()).astype(np.int16)
    ...
    core = {
        "meta": {
            "title": "LPS v5.4 ERA5 South Asian low-pressure-system catalogue",
            "rows": int(len(data)),
            ...
            "row_grain": "One row per hourly linked LPS position/time, with complete ERA5 physics resampled at the published centre.",
            "source_dataset": "ERA5-derived LPS v5.4 core catalogue",
            "catalogue_version": "v5.4",
            "schema": "lps-v5.4-era5-core-v1",
            "atlas_version": "3.3.0",
            "built_utc": built_utc,
            ...
            "sources": {
                ...
                "release_summary": "data/lps-v5.4-era5-1940-2025-core.metadata.json",
    ...
    detail = {
        ...
        "state_rainfall": {
            ...
            "units": "mm day-1",
            "statistic": "Area-mean daily rainfall averaged over UTC calendar days touched by each LPS track.",
            "coverage": [1940, 2025],
    ...
    manifest = {
        ...
        "qa": {
            ...
            "all_rows_have_published_centre_physics": bool(not data[physics].isna().any().any()),
            "all_rows_have_v54_geometry": bool(not data[["lon", "lat"]].isna().any().any()),
            "release_gates_passed": bool(metadata["qa"]["all_release_gates_passed"]),
            "state_rainfall_columns": int(sum(state_available)),
            "tracks_with_state_rainfall": int(np.any(state_matrix >= 0, axis=1).sum()),
