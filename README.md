...
The initial four-pair review covers MPI-ESM1-2-HR, MPI-ESM1-2-LR, MIROC6 and MRI-ESM2-0. Structural QA passes for every run, but the publication review remains on hold: MIROC6 alone passes the conservative all-LPS historical-climatology rubric, and none reproduces ERA5 depression-or-stronger frequency in both all-month and JJAS views within a factor of two. Track-density correlation and probability overlap are reported separately from counts, and absolute category realism is separated from all-LPS track realism. The generated JSON and Markdown review therefore recommend an initial climate view only after at least one more independent model passes, with D/DD/CS changes withheld from headline interpretation. The unanimous preliminary signal is increased track-centred 24-hour precipitation, but it remains a staging result.
