...
`cmip6_pipeline.standardise_era5` supplies that resolution control by sampling ERA5 at the exact common 1-degree nodes, recomputing spherical vorticity only after sampling, and adding actual hourly surface pressure from the CEDA/BADC `lnsp` analysis. The unchanged detector, linker, physical-event gate and intensity classifier then run over 1981–2010. `cmip6_pipeline.impact` independently builds one-event-one-vote storm-centred trailing-24-hour precipitation footprints and India-wide JJAS rainfall attribution within 800 km, including exposure, climatological-excess and heavy-rain diagnostics on each model's native calendar.
