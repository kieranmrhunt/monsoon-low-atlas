...
Latest files include positive 850-hPa relative vorticity and trailing-24-hour precipitation on a common 1-degree grid. Relative vorticity is derived from winds only after every provider is resampled to that grid; this avoids mixing IFS native spectral vorticity with wind-derived AIFS/GFS fields. Ensemble weather is the arithmetic member mean, while individual member tracks are shown by default whenever an ensemble run is selected and remain user-toggleable. Clicking a disturbance opens a linked evolution chart: centre diagnostics supply the hourly member-mean vorticity line and the displayed deterministic or ensemble-mean weather field supplies the track-centred trailing-24-hour precipitation bars; its marker shares the map valid-time slider. Full weather-capable payloads are retained for every six-hourly initialization in a rolling 48-hour window, but lightweight track-only sidecars are fetched first so unused weather grids do not delay the map. There is no artificial comparison cap: the client can compare every selected model at one initialization or all available neighbouring initializations for one model, with weather still tied to one explicitly labelled run. AIGEFS retries whole members once after transient NOMADS redirect/throttle failures while retaining the operational 70% ensemble publication floor.
