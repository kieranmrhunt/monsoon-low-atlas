...
The GWL timing layer uses the public IPCC WGI AR6 Atlas CMIP6 crossing table (Iturbide et al., 2022, CC BY 4.0). Its central year is the first centred 20-year mean to reach 1.5, 2, 3 or 4 °C above 1850–1900, so a central year `n` denotes years `n-9` through `n+10`. The exact five SSP2-4.5 model/member runs in the research preview are all represented. `cmip6_pipeline.gwl` converts those published timings into source-validated model-native processing plans, and `scripts/submit_cmip6_gwl.sh` runs the frozen detector/linker on the 1.5 and 2 °C windows. Nine windows lie wholly in ScenarioMIP and can run directly. HadGEM3-GC31-LL's 2010–2029 1.5 °C window crosses the historical/SSP boundary and is intentionally held for explicit cross-experiment stitching rather than being truncated or assigned to the wrong experiment.
