...
The main map's Track analysis control can show the ERA5 v5.6 catalogue geometry, a confidently matched MERRA-2, IMDAA, JRA-55 or ERA-Interim geometry, or all available analyses together. Alternative reanalyses are run through the same frozen v5.4.2 liberal candidate detector and continuity-linker parameter file retained by v5.6. In the main Explorer, validity comes from the published ERA5 event: the normal match gate requires at least 12 common hours, median and 90th-percentile separations of at most 300 and 500 km, at least half of common hours within 250 km, and an unambiguous best ERA5 identity. A physically selected source event can also recover a long, coherent match when both tracks overlap by at least 50%, at least 70% of common hours are within 250 km, median separation is at most 150 km and the 90th percentile is at most 800 km. Assignment favours a physically selected, well-covered source identity over a short close fragment, and displayed source geometry is clipped to the ERA5 event lifetime. In Forecast Archive view, where ERA5 matching is deliberately unnecessary, a source-native track instead has to pass the frozen detector-space physical-event thresholds for duration, observed support, mature in-domain centres, vorticity, pressure deficit, candidate quality, centre score and heat-low rejection. The source-native runs do not repeat ERA5's subsequent final-centre resampling, closed-isobar calculation or circulation-wind intensity classification, and their input cadence and available diagnostics remain source-specific. This comparison therefore does not replace the ERA5 catalogue classification.

MERRA-2 uses NASA GES DISC M2I3NPASM, M2I1NXASM and M2T1NXFLX subsets from 1980 through the latest complete common month. IMDAA requests cover its official 1979–2020 archive through the NCMRWF Research Data Service. JRA-55 uses regional NCSS subsets of NCAR GDEX d628000 for 1958–January 2024, while ERA-Interim reads the complete local BADC archive for 1979–August 2019. Every source is standardized to the common one-degree detector contract, tracked natively, physically selected, matched to the public ERA5 parquet file and published with checksums and explicit coverage. The main atlas loads only confidently matched geometries. Forecast Archive view instead loads a small monthly source-native asset and can draw every physically selected reanalysis track active on the selected UTC date, whether or not that track has an ERA5 identity match; ERA5 remains a separate forecast-verification overlay.

Submit and validate one MERRA-2 month with:

```bash
bash scripts/submit_merra2_month.sh 2016-07
python -m unittest discover -s reanalysis_pipeline -t . -p 'test_*.py'
```
...
The full-period backfills are resumable Slurm arrays submitted by `scripts/submit_merra2_backfill.sh`, `scripts/submit_jra55_backfill.sh` and `scripts/submit_era_interim_backfill.sh`; the guarded IMDAA pump is `scripts/pump_imdaa_backfill.sh`. Multi-decadal candidate collections use the same overlapping annual-block strategy as v5.6 production: each core year includes December/January halos, blocks link independently, and shared observed candidate identities reconcile cross-year tracks one-to-one before ERA5 matching. The JRA-55 downloader validates requested variables, pressure level and time range before accepting each NCSS response.

## CMIP6 climate-track inventory

`scripts/inventory_badc_cmip6.py` audits high-frequency CMIP, ScenarioMIP and HighResMIP holdings under `/badc/cmip6/data/CMIP6`. Its model/experiment/member table distinguishes verified fixed-pressure candidates, hybrid-level runs that need vertical interpolation and incomplete holdings; it does not call a model trackable from filenames alone. The accompanying climate-tab contract preserves native-calendar identity, pairs like-for-like historical and scenario members, defaults to one-model-one-vote summaries and requires a month-scale QA pilot before full tracking.
...
bash scripts/submit_cmip6_pair.sh mri-canary
bash scripts/submit_cmip6_pair.sh hadgem-ll-canary
bash scripts/submit_cmip6_pair.sh hadgem-ll-paired
bash scripts/submit_cmip6_pair.sh hadgem-mm-ssp126-canary
bash scripts/submit_cmip6_pair.sh hadgem-mm-ssp585-canary
bash scripts/submit_cmip6_gwl.sh ssp245
python -m cmip6_pipeline.warming build --run-root .cmip6-runs/mri-paired --run-root .cmip6-runs/mpi-esm1-2-hr-production --run-root .cmip6-runs/hadgem-ll-paired --run-root .cmip6-runs/mpi-lr-paired --run-root .cmip6-runs/miroc6-paired --output-dir .cmip6-runs/global-warming
python -m unittest discover -s cmip6_pipeline -t . -p 'test_*.py'
python -m cmip6_pipeline.review --manifest .cmip6-runs/multimodel-review/manifest.json --output-dir .cmip6-runs/multimodel-review/review
```

MIROC6, MPI-ESM1-2-LR, MRI-ESM2-0 and HadGEM3-GC31-LL use separate four-month historical/future canaries before their 30-year presets are eligible to run. MRI's future production window is 2070–2099 because its archive has no 2101 pressure-level boundary frame; the initial Gregorian pairs otherwise use 2071–2100. Pressure-level fields that lie below a model's topography are kept missing rather than extrapolated. If any required final-centre field is unavailable within an interpolated bridge, the complete bridge is rejected and its observed portions remain separate events; this prevents high-terrain gaps from acquiring invented 850-hPa structure. Annual-block identities are also split, without creating centres, wherever retained rows cease to be exactly hourly or exceed the final 150-km hourly displacement limit. Any resulting fragment containing only posterior interpolation and no actual detection is discarded and counted in the gap audit. Once both windows in any production pair pass, the final jobs validate track-hour uniqueness, hourly continuity, physical support, domain bounds and centre displacement; the historical run is also screened against ERA5 v5.6 over the identical years and calendar months for frequency, system-days, duration, path length, circulation wind, pressure deficit, seasonal cycle, track-density shape and absolute category rates. These diagnostics identify model and resolution bias but never retune the detector or intensity thresholds. The jobs then build seasonal run summaries, year-resampling uncertainty and a relocatable browser bundle under the run's `climate-public/` staging directory. The public climate tab accepts the explicitly labelled `multi-model-awaiting-review` research preview as well as a future `validated-production-window`, and exposes historical bias beside projected change.

Non-Gregorian integrations are carried through the unchanged pandas-based detector on a strictly ordinal analysis clock: every interval is the same elapsed model hour, and no model day is inserted, removed, stretched or relabelled as a civil date. Final catalogues restore the exact native date as `model_time` plus numeric native-calendar components, retain the analysis timestamp separately, and QA recomputes every mapping in both directions. Unit tests cover Gregorian leap days, no-leap calendars and 360-day dates including 30 February. The HadGEM3-GC31-LL 360-day historical/SSP2-4.5 canary and complete 30-year pair pass the structural field, tracking, physics and native-time QA chain. Its historical and future halves consistently use the available six-hour interval-mean MSLP table—recorded through `cell_methods` and provenance—because the future archive lacks instantaneous MSLP. Its complete windows are 1981–2010 and 2070–2099. The model is included in the five-model research preview, with its historical frequency bias shown explicitly rather than treated as validated realism.

`python -m cmip6_pipeline.summarise ensemble-index` merges two or more 30-year public staging manifests into an equal-weight candidate. It averages annual rates, monthly climatology, class shares and per-year track density once per source model; paired-change uncertainty resamples years within each model, while model spread is reported separately. The headline percentage is the change between the displayed equal-weight historical and future means; the arithmetic mean of individual model percentage changes is retained only as a secondary diagnostic. The track-density product also retains the positive, negative and unchanged vote from every model in every 1-degree cell, allowing the browser to distinguish mean change from sign agreement. The output remains `multi-model-awaiting-review`. Changing it to `validated-production-window` requires an explicit approved review JSON naming the exact included pair IDs, approver and timestamp.

`cmip6_pipeline.warming` calculates global-mean near-surface air-temperature change from monthly `Amon tas` for the same model, member and exact historical/future windows as every track pair. It uses decoded native-calendar time bounds and exact spherical areas from latitude/longitude cell bounds, validates complete monthly coverage, and records the source files and annual means. Attaching the registry to a climate bundle adds model-specific warming without altering the paired LPS estimates. The atlas then reports a separate percentage response per degree Celsius; the equal-model value is the arithmetic mean of the independently normalised model responses, while each whisker remains that model's within-model annual-resampling interval divided by its own warming.

The GWL timing layer uses the public IPCC WGI AR6 Atlas CMIP6 crossing table (Iturbide et al., 2022, CC BY 4.0). Its central year is the first centred 20-year mean to reach 1.5, 2, 3 or 4 °C above 1850–1900, so a central year `n` denotes years `n-9` through `n+10`. The exact five SSP2-4.5 model/member runs in the research preview are all represented. `cmip6_pipeline.gwl` converts those published timings into source-validated model-native processing plans, and `scripts/submit_cmip6_gwl.sh` runs the frozen detector/linker on the 1.5 and 2 °C windows. HadGEM3-GC31-LL's 2010–2029 1.5 °C window crosses the historical/SSP boundary: its fields are joined before detection on one continuous 360-day analysis clock, so a system may retain identity across 2015. `scripts/submit_cmip6_gwl_impacts.sh` reuses each model's fixed 1981–2010 rainfall baseline and computes the corresponding storm-centred and India-wide precipitation diagnostics for every completed GWL window.

The initial five-pair review covers HadGEM3-GC31-LL, MPI-ESM1-2-HR, MPI-ESM1-2-LR, MIROC6 and MRI-ESM2-0. Structural QA passes for every run, but formal validation remains on hold: MIROC6 alone passes the conservative all-LPS historical-climatology rubric, and none reproduces ERA5 depression-or-stronger frequency in both all-month and JJAS views within a factor of two. Track-density correlation and probability overlap are reported separately from counts, and absolute category realism is separated from all-LPS track realism. The public research preview therefore defaults to an equal-weight five-model view with a model-by-model change interval and an adjacent ERA5 historical-agreement panel. JJAS system counts split two increases to three decreases with no individual interval excluding zero; mean peak 24-hour precipitation increases in all five models and every individual interval excludes zero. D/DD/CS changes remain secondary because the completed common-grid ERA5 control shows that absolute intensity categories are substantially more resolution-sensitive than all-LPS tracks.

`cmip6_pipeline.standardise_era5` supplies that resolution control by sampling ERA5 at the exact common 1-degree nodes and recomputing spherical vorticity only after sampling. Pressure-level terrain validity uses the tracker's fixed standard-atmosphere estimate from hourly ERA5 MSLP and common ERA5 orography because the migrated CEDA/BADC `lnsp` tree is incomplete during the control window; a month with actual `lnsp` gives a 0.19-hPa median absolute error and changes only 0.60%, 0.24% and 0.29% of grid-hour validity decisions at 850, 700 and 500 hPa. The unchanged detector, linker, physical-event gate and intensity classifier then run over every hour of 1981–2010. The completed control passes structural QA and finds 738 events against 706 in native ERA5: all-month event frequency differs by 4.5%, JJAS by 8.8%, median duration by less than 1% in JJAS, and JJAS track-density pattern correlation is 0.982. By contrast, the 1-degree control retains only 55% of native JJAS depression-or-stronger frequency and 31% of deep-depression-or-stronger frequency, with no JJAS cyclonic-storm classifications. This isolates coarse-grid intensity attenuation without suggesting that the frozen tracker loses the underlying LPS population. The black point in the atlas historical-agreement panel exposes the same control alongside each model.

`cmip6_pipeline.impact` independently builds one-event-one-vote storm-centred trailing-24-hour precipitation footprints and India-wide JJAS rainfall attribution within 800 km, including exposure, climatological-excess and heavy-rain diagnostics on each model's native calendar. The same diagnostics are calculated for six fixed, non-overlapping mainland macro-regions assembled from the atlas state/UT geometry; 276 of the 277 national 1-degree land-cell centres fall in those regions, with the single island cell retained in the national result. In the initial equal-weight five-model SSP2-4.5 set, all-India JJAS rain rises 12.2% and rain within 800 km of LPS centres rises 11.7%; all five models increase both measures. The raw fraction of Indian rain on exposed cell-days rises only 0.26 percentage points, however, and its 90% interval crosses zero because total seasonal rain intensifies at nearly the same rate. The equal-weight storm-centred footprint increases by 9.8% over its full ±10-degree domain and by 14.4% at the storm centre. These are paired model responses, not observational attribution estimates.

The public climate-change tab is explicitly marked as under construction and is divided into overview, tracks and pathways, rainfall and impacts, storm structure, and evaluation and availability views. Its summary contract currently exposes 59 source-backed measures in eight scientific families, including counts and classes, lifecycle and displacement, wind and pressure, three-level vorticity, precipitation, moisture, temperature and core structure, and environmental context. Every chart reports its own contributing model count; unavailable model/variable/season combinations are disabled or left missing rather than inferred. Comparison, season, measure, spatial diagnostic, profile and view are encoded in the shareable URL, and the selected model-change table can be exported as CSV or the primary chart as PNG.

The HighResMIP full-physics comparison uses `hist-1950` and `highres-future` pairs for CNRM-CM6-1, EC-Earth3P and EC-Earth3P-HR. Canary windows comparing JJAS 1990 with JJAS 2030 preceded the validated 1981–2010 against 2020–2049 production catalogues. These experiments remain a distinct mid-century HighResMIP comparison: they are not pooled into ScenarioMIP or global-warming-level ensembles.
...
Displayed forecast paths prepend a compact analysis history when a system is already present at initialization. The service retains 14 days of six-hourly t+0 centres for active guidance and embeds t+0 centres in each processed archive entry. Matching is model- and operational-version-specific: an older forecast signature must reproduce the newer analysed centre, pass distance and motion checks, and be unambiguous relative to contemporaneous systems. The stitched analysis history is a thin solid line with small centre markers, and the complete forecast is a thicker solid line; the time slider moves the marker without restyling the path. Forecast-only genesis therefore remains unstitched, and historical spacing reflects the archive cycles actually processed rather than implying unavailable six-hourly analyses.
...
The production wrapper submits ten direct weather-bearing feeds as independent Slurm jobs and one `afterany` finalizer, then launches AIGEFS and WeatherNext 2 as resumable member shards. The finalizer validates and merges every completed model into the public manifest atomically; a failed model retains its previous Latest cycle. The wrapper submits no duplicate while that finalizer is queued or running:

```bash
bash scripts/submit_forecast_update.sh
```

WeatherNext 2 access on each compute node uses Google Application Default Credentials (or a service-account file named by `GOOGLE_APPLICATION_CREDENTIALS`). Its rolling 72-hour window can be repaired independently with `bash scripts/submit_weathernext2_shards.sh recent`; completed members are reused on the retry pass and each successful cycle is added to the permanent Archive as well as Latest.
