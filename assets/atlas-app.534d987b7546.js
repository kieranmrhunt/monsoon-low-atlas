(async function () {
	...
	function renderDossier() {
		...
		const facts = [
			...
			['Observed centres', fmt(row[T.observed_positions])],
			['Interpolated centres', `${fmt(row[T.posterior_fraction_x1000] / 10, 1)}%`],
			['Publication segments', fmt(row[T.stitch_count])],
		...
		node.innerHTML = `
			<div class="mla-dossier-head"><div><div class="mla-badge-row">${badges.join('')}</div><h3>${esc(systemLabel(index))}</h3><p class="mla-dossier-sub">${date(row[T.start_ms])} to ${date(row[T.end_ms])} · parent event ID ${atlasId(index)}</p></div><button class="mla-btn mla-btn-icon mla-btn-small" id="mlaClearSelection" type="button" aria-label="Clear selected track">×</button></div>
			<div class="mla-fact-grid">${facts.map(fact => `<div class="mla-fact"><span>${esc(fact[0])}</span><strong>${esc(fact[1])}</strong></div>`).join('')}</div>
			<p class="mla-caution"><strong>Continuity:</strong> v5.4.1 re-samples ERA5 physics at every published centre. Hard line breaks separate unsupported plotting stitches and are preserved in downloads.</p>
	...
	function renderLifeCharts() {
		...
		if (state.selected == null) {
			...
			$('#mlaLifeData').innerHTML = accessibleTable(['Hours since genesis', `${definition.title} (${definition.unit})`, '24 h rain (mm)'], rows, 'Physics is resampled at each published v5.4.1 centre; hard plotting breaks are not bridged.');
		}
		if (!DETAIL) {
	...
	function summaryRows(indexes) {
		return indexes.map(index => {
			const row = track(index);
			return {
				continuity_parent_track_id: atlasId(index),
				...
				publication_segment_count: row[T.stitch_count],
	...
	function reproducibilityState() {
		return {
			...
			caveats: [
				...
				'Interpolated positions preserve supported continuity; v5.4.1 physics is resampled at every published centre.',
				'Hard line breaks represent unsupported stitches between publication segments.',
	...
	async function downloadSelectedFixes() {
		...
		const headers = ['continuity_parent_track_id', 'publication_segment_number', 'time_utc', 'hours_since_genesis', 'latitude', 'longitude', 'position_source', 'precip_24h_mm', 'vorticity_1e-5_s-1', 'max_wind_ms', 'mslp_hpa', 'pressure_deficit_hpa', 'q850_gkg', 'rh850_pct', 't850_k', 'atlas_class'];
		const rows = series[S.hours_since_genesis].map((hour, pointIndex) => {
			if (breaks.has(pointIndex)) segmentNumber++;
			return [
			atlasId(index),
			segmentNumber,
	...
	function renderData() {
		...
		$('#mlaBuildText').textContent = `Atlas ${CORE.meta.atlas_version}, built ${CORE.meta.built_utc}; ${fmt(CORE.meta.tracks)} parent events, ${fmt(CORE.meta.publication_segments)} complete plotting segments and ${fmt(CORE.meta.rows)} hourly positions.`;
