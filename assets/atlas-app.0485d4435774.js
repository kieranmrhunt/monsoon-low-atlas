(async function () {
	...
	function pointSegmentKm(lon, lat, a, b) {
		...
		const bx = b[0] * cosine;
		const by = b[1];
		return Math.sqrt(pointSegmentDistanceSquared(px, py, ax, ay, bx, by)) * 111.2;
	}

	function isTrackNearState(trackIndex, stateIndex, radiusKm) {
		const key = `${stateIndex}:${radiusKm}`;
		...
		const latitudePad = radiusKm / 111.2;
		const longitudePad = radiusKm / (111.2 * Math.max(.35, Math.cos(((bbox[1] + bbox[3]) / 2) * Math.PI / 180)));
		const trackBounds = CORE.bounds[trackIndex];
		if (trackBounds[2] < bbox[0] - longitudePad || trackBounds[0] > bbox[2] + longitudePad || trackBounds[3] < bbox[1] - latitudePad || trackBounds[1] > bbox[3] + latitudePad) {
			cache[trackIndex] = 0;
			return false;
		}
		const points = paths.decoded[trackIndex];
		let near = false;
		for (let pointIndex = 0; pointIndex < points.length && !near; pointIndex += Math.max(1, Math.floor(points.length / 100))) {
			const point = points[pointIndex];
			for (const ring of geometry.rings) {
				if (pointInRing(point[1], point[0], ring)) { near = true; break; }
				for (let vertex = 1; vertex < ring.length; vertex += Math.max(1, Math.floor(ring.length / 120))) {
					if (pointSegmentKm(point[1], point[0], ring[vertex - 1], ring[vertex]) <= radiusKm) { near = true; break; }
				}
				if (near) break;
			}
		}
		cache[trackIndex] = near ? 1 : 0;
		return near;
	}
	...
	function statePass(index) {
		...
		if (!CORE.state_available[state.stateIndex]) return false;
		const row = DETAIL.state_pct[index];
		return row && row[state.stateIndex] >= state.stateMin && isTrackNearState(index, state.stateIndex, 800);
	}
	...
	function buildFilterControls() {
		...
		CORE.states.forEach((name, index) => {
			...
			option.textContent = CORE.state_available[index] ? name : `${name} — unavailable`;
			option.disabled = !CORE.state_available[index];
	...
	function bindControls() {
		...
		$('#mlaState').addEventListener('change', async event => {
			state.stateIndex = event.target.value === '' ? -1 : Number(event.target.value);
			if (state.stateIndex >= 0) await ensureDetail('Opening state-rainfall context…');
	...
	window.addEventListener('popstate', async () => {
		suppressUrl = true;
		readUrl();
		if (state.stateIndex >= 0) await ensureDetail();
		syncControls();
	...
	function mapLegend(layer, maximum) {
		...
		if (layer === 'density') {
			node.innerHTML = `<span class="mla-legend-item"><span class="mla-swatch" style="background:${ramp(.2)}"></span>fewer</span><span class="mla-legend-item"><span class="mla-swatch" style="background:${ramp(1)}"></span>up to ${fmt(maximum)} tracks/cell</span>`;
			return;
		}
		if (state.mapColour === 'class') {
		...
		node.innerHTML = `<span class="mla-legend-item"><span class="mla-swatch" style="background:${ramp(.1)}"></span>low</span><span class="mla-legend-item"><span class="mla-swatch" style="background:${ramp(.55)}"></span>middle</span><span class="mla-legend-item"><span class="mla-swatch" style="background:${ramp(1)}"></span>high</span>`;
	}
	...
	function downloadGeojson() {
		...
		toast(`Exported ${fmt(features.length)} gap-aware tracks`);
	}
